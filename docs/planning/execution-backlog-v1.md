...
## 7.1 Текущие открытые задачи (2026-06-13)

### AI layer (E5)

- [ ] **5c.5.2 INFO-лог chief-контекста** (nice-to-have) — логировать
  отрендеренный контекст chief (с секцией `=== subagent_reports ===`)
  для прямой наблюдаемости синтеза, без сопоставления content post-factum.
- [ ] **`list_latest_agent_runs` без LIMIT** — объединить с задачей
  retention/индекс `ops.ai_agent_runs` (MP1). Сейчас LIMIT 2000 на
  роль — достаточно для v1, но без индекса запрос будет деградировать
  при росте таблицы.
- [ ] **FOOTGUN E** — `LiteLLMModelClient` захватывать HTTP status +
  тело ответа в error-текст.
- [ ] **FOOTGUN F** — SSE-роуты (stream) пустые: либо backend начинает
  периодически пушить snapshot-события, либо frontend переключается
  на polling `/overview` как fallback.

### UI (E3/E4)

- [ ] **UI-трек** — нарезка по фазам после аудита скриншотов и SSE-RECON.
